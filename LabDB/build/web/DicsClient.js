...
// Model for Dics entity
    models.Dics = Backbone.Model.extend({
        urlRoot: "http://localhost:8084/webresources/entities.dics/",
        idAttribute: 'id',
        defaults: {
            name: ""
        },
        ...
        sync: function(method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8084/webresources/entities.dics/';
            }
    ...
    // Collection class for Dics entities
    models.DicsCollection = Backbone.Collection.extend({
        model: models.Dics,
        url: "http://localhost:8084/webresources/entities.dics/",
        sync: function(method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function(jqXHR, textStatus, errorThrown) {
...
$(function() {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function() {
            this.collection = new models.DicsCollection();
            var self = this;
            this.collection.fetch({
                success: function() {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-dics-list-itemis template identifier for item
                        templateName: '#tpl-dics-list-item'
                    });
        ...
        details: function(id) {
            if (this.collection) {
                this.dics = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.dics,
                    // tpl-dics-details is template identifier for chosen model element
                    templateName: '#tpl-dics-details',
        ...
        create: function() {
            ...
            var self = this;
            var dataModel = new models.Dics();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-dics-details is a template identifier for chosen model element
                templateName: '#tpl-dics-details',
